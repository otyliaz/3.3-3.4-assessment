...
<?php
//select everything in product table
$query = "SELECT * FROM product";

if (isset($_GET['search'])){ //if they press search
  //trim off whitespace from search term
  $search_term = trim($_GET['search']);

  if (!empty($search_term)) {
    // if search_term is not null or whitespace,
    $query .= " WHERE name LIKE '%$search_term%'"; //adds WHERE clause to original query
  }
}
...
//loop through products and store in an array
...
if (!empty($search_term)) {
  // if search_term is not null or whitespace,
  echo ' for "'.$search_term.'"';} //adds the search term to the result

echo ':</p>';


if (isset($_SESSION['admin'])) {
  //if user is admin, show add product button
  echo "
